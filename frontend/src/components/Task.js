...
function Task() {
    ...
    return (
        <div className='px-6 py-4 w-full h-full overflow-hidden'>
        <div className="flex flex-col md:flex-row items-center justify-between mb-6">
          {/* Project Title and Dropdown */}
        ...
        <DragDropContext onDragEnd={result => onDragEnd(result, columns, setColumns)}>
          <div className="flex flex-col md:flex-row gap-5 overflow-auto">
            {Object.entries(columns).map(([columnId, column], index) => {
              return (
                <div className="w-full md:w-3/12 min-h-[580px]" key={columnId}>
                  {/* Column Title */}
                  <div className="pb-2.5 w-full flex justify-between items-center">
                    ...
                    {(provided, snapshot) => {
                      return (
                        <div
                          ...
                          className={`min-h-[530px] pt-4 transition-colors border-t-2 border-indigo-400 ${snapshot.isDraggingOver && 'border-indigo-600'}`}
                        >
